...
CREATE TABLE Result (
  ...
  CONSTRAINT CK_Result_Status CHECK (status IN ('Finished', 'DNF', 'DNS', 'DQ')),
  CONSTRAINT CK_Result_OverallPosition CHECK (
    overall_position IS NULL
    OR overall_position > 0
  ),
  CONSTRAINT CK_Result_CategoryPosition CHECK (
    category_position IS NULL
    OR category_position > 0
  ),
...
VALUES
  ...
  (
    'Walking',
    'Competitive and recreational walking events'
  ),
...
VALUES
  (
    'Johannesburg Running Club',
    'JRC001',
    'info@jrc.co.za',
    '0115551001'
  ),
  (
    'Gauteng Cycling Club',
    'GCC001',
    'info@gcc.co.za',
    '0115551002'
  );
